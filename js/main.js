...
// nudged up +0.06 total (two +0.03 passes) per feedback that the front
// lighting, hazards, and flag were all sitting a bit low. headlightSpot's y
// raised by the same amount to keep the actual light source aligned with
// the (now higher) visible lens; headlightSpotTarget (far-ahead aim point)
// and reverseLight are unaffected — same reasoning as the z-only
// headlightSpot nudge below.
const TRUCK_LAYOUT = {
	headlightLens: [ 0.3975, 0.359, 1.4 ],
	taillight: [ 0.3975, 0.489, -1.33 ],
	reverseLight: [ 0.24975, 0.429, -1.3398 ],
	flag: [ -0.6, 0.203, -1.358 ],
	...
	headlightSpot: [ 0.3975, 2.1595, 1.7612 ],
	headlightSpotTarget: [ 0.3975, 0.2002, 18.004 ],
	hazards: [
		[ -0.3975, 0.359, 1.4 ], [ 0.3975, 0.359, 1.4 ],
		[ -0.3975, 0.489, -1.33 ], [ 0.3975, 0.489, -1.33 ],
	],
};
...
// ✏️ Same +0.06 total lift as TRUCK_LAYOUT above (headlightLens/taillight/
// hazards/flag y, and headlightSpot y to match) — see its comment for why.
const CAMRY_LAYOUT = {
	headlightLens: [ 0.6535, 0.5798, 2.214 ],
	taillight: [ 0.7639, 0.8348, -2.3324 ],
	reverseLight: [ 0.4797, 0.7748, -2.3496 ],
	flag: [ -0.8744, 0.0942, -2.5813 ],
	...
	headlightSpot: [ 0.6535, 2.4618, 2.8763 ],
	headlightSpotTarget: [ 0.6535, 0.1034, 33.1454 ],
	hazards: [
		[ -0.6535, 0.5798, 2.214 ], [ 0.6535, 0.5798, 2.214 ],
		[ -0.7639, 0.8348, -2.3324 ], [ 0.7639, 0.8348, -2.3324 ],
	],
};
...
// hazards/flag y, and headlightSpot y to match) — see its comment for why.
const CAMARO_LAYOUT = {
	headlightLens: [ 0.3584, 0.396, 1.3312 ],
	taillight: [ 0.3123, 0.4476, -1.5104 ],
	reverseLight: [ 0.1961, 0.3876, -1.5215 ],
	flag: [ -0.4603, 0.1462, -1.4774 ],
	...
	headlightSpot: [ 0.3584, 1.3319, 1.7232 ],
	headlightSpotTarget: [ 0.3584, 0.1209, 17.8476 ],
	hazards: [
		[ -0.3584, 0.396, 1.3312 ], [ 0.3584, 0.396, 1.3312 ],
		[ -0.3123, 0.4476, -1.5104 ], [ 0.3123, 0.4476, -1.5104 ],
	],
};
